...
func (ds *download) Add(ctx context.Context, d *data.Download) (*data.Download, bool, error) {
    ...
    if saved.Status == data.StatusActive && created {
        // Propagate request_id into background start if present.
        ...
        go func(d *data.Download, cctx context.Context) {
            ...
            if derr != nil {
                ...
                _, _ = ds.repo.Update(cctx, d.ID, func(dl *data.Download) error {
                    dl.Status = data.StatusError
                    return nil
                })
                return
            }
            _, err := ds.repo.Update(cctx, d.ID, func(dl *data.Download) error {
                dl.GID = gid
                return nil
            })
            if err != nil {
...
func (ds *download) Delete(ctx context.Context, id string, deleteFiles bool) error {
    ...
    // Cancel any in-flight start operation to avoid racing with deletion.
    ds.startMu.Lock()
    ...
    // Delegate all cancellation and file cleanup to the downloader adapter.
